...
def first_process(union_df, match_out_col, match_in_col, in_col, out_col, union_yet_outdir, union_not_outdir):
    """
    第一次生成，输入只有进项表和销项表。输出有两种可能。1、临时完成表+临时剩余表+待填充表。2、匹配完成表+匹配剩余表。
    :return:
    """
    process_yet(union_df, match_out_col, match_in_col, in_col, out_col, union_yet_outdir)
    # 第一次匹配，生成匹配完成表
    process_not(union_df, match_out_col, match_in_col, in_col, out_col, union_not_outdir)
    # 第二次匹配，生成匹配剩余表
    return


def process_yet(union_df, match_out_col, match_in_col, in_col, out_col, union_yet_outdir):
...
def process_not(union_df, match_out_col, match_in_col, in_col, out_col, union_not_outdir):
